...
export default function PlanComparison({ plugin }: Props) {
  ...
  const leftCost = useMemo(
    () => (leftPlan ? calculatePlanComparison(leftPlan, monthlyKwh, peakPercent, season) : 0),
    [leftPlan, monthlyKwh, peakPercent, season]
  );
  const rightCost = useMemo(
    () => (rightPlan ? calculatePlanComparison(rightPlan, monthlyKwh, peakPercent, season) : 0),
      ...
      {savings > 5 && (
        <div className="bg-green-50 border border-green-200 rounded-lg px-4 py-3 text-center">
          <span className="text-green-700 font-medium">
            {cheaperSide === "left" ? leftPlan?.name : rightPlan?.name} saves you{" "}
