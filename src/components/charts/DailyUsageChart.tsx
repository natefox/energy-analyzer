...
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, Brush, ReferenceLine } from "recharts";
import type { DailyData } from "@/lib/types";
import { formatDate, formatKwh, periodColor } from "@/lib/utils";

interface Props { data: DailyData[]; hasMidPeak?: boolean; hasSolar?: boolean; }
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData}>
          ...
          {hasMidPeak && <Bar dataKey="midPeakUsage" name="Mid-Peak" stackId="usage" fill={periodColor("midPeak")} />}
          <Bar dataKey="offPeakUsage" name="Off-Peak" stackId="usage" fill={periodColor("offPeak")} />
          <Bar dataKey="superOffPeakUsage" name="Super Off-Peak" stackId="usage" fill={periodColor("superOffPeak")} />
          {hasSolar && (
            <>
              <ReferenceLine y={0} stroke="#666" />
              <Bar dataKey="solarGeneration" name="Solar Generation" stackId="solar" fill="#eab308" />
            </>
          )}
          {data.length > 30 && <Brush dataKey="date" height={30} stroke="#8884d8" tickFormatter={formatDate} />}
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
