...
interface Props { data: DailyData[]; hasMidPeak?: boolean; }
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={data}>
          ...
          <Tooltip labelFormatter={(label) => formatDate(String(label))} formatter={(value) => [formatCurrency(Number(value)), String(value)]} />
          <Legend />
          <Bar dataKey="peakCost" name="Peak" stackId="cost" fill={periodColor("peak")} />
          {hasMidPeak && <Bar dataKey="midPeakCost" name="Mid-Peak" stackId="cost" fill={periodColor("midPeak")} />}
          <Bar dataKey="offPeakCost" name="Off-Peak" stackId="cost" fill={periodColor("offPeak")} />
          <Bar dataKey="superOffPeakCost" name="Super Off-Peak" stackId="cost" fill={periodColor("superOffPeak")} />
